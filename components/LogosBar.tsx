...
const clients: Client[] = [
  { name: "MDM Mobiliario", img: "/clients/mdm.jpg", imgClass: "h-10 md:h-12 w-auto object-contain rounded" },
  { name: "Cliente 2", img: "/clients/client2.png", imgClass: "h-8 md:h-10 w-auto object-contain" },
  { name: "Iris Studio" },
  { name: "Eventicos" },
  { name: "Villa Solstice" },
  { name: "Ezelandscape" },
    ...
    <section className="border-y border-white/5 bg-carbon-gray py-10 md:py-12">
      <div className="max-w-[1600px] mx-auto px-6 md:px-12">
        ...
        <div className="overflow-hidden">
          <div className="flex items-center animate-scroll gap-10 md:gap-16">
            {[...clients, ...clients].map((client, i) =>
              client.img ? (
                <img
                  ...
                  src={client.img}
                  alt={client.name}
                  className={`shrink-0 ${client.imgClass} opacity-60 hover:opacity-100 transition-opacity`}
                />
              ) : (
                <span
                  key={`${client.name}-${i}`}
